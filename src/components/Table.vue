<template>
    <v-data-table :headers="headers" :items="tableData" :items-per-page="10" sort-by="value" sort-desc class="elevation-2 ma-2">
        <template v-slot:top>
            <v-toolbar flat rounded color="transparent">
                ...
                <v-dialog v-model="dialog" max-width="400px">
                    <template v-slot:activator="{ on, attrs }">
                        <v-btn :color="getColor" class="pa-2" v-bind="attrs" v-on="on"><v-icon>mdi-plus</v-icon>Add {{ type }}</v-btn>
                    </template>
